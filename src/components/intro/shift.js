...
const drawScene = time => {
  if (canvasContext) {
    ...
    planes.forEach((plane, pi) => {
      const n = (plane.noise.simplex2(time / 500, 0) + 1) / 2
      const shift = n * 200
      plane.rects.forEach((rect, ri) => {
        ...
        const top = h - rect.height * h
        const m = 10
        const r = 200
        const lines = Math.floor((rect.height * h) / m)
        sequence(lines).forEach(i => {
          canvasContext.fillRect(
            Math.floor(rect.x * w) + (i > lines / 3 && i < (lines * 2) / 3 ? shift : 0),
            Math.floor(top + i * m),
...
const animatePlane = (plane, planeIndex) => {
  ...
  return {
    ...
    delay: anime.stagger(400, {
      easing: "easeOutQuad", //randomItem(["easeInQuad", "easeInCubic"]),
